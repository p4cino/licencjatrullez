...
<div class="container text-center">
	...
	function rezerwacjaToru($id_uzytkownika, $godzina, $tor, $data){
		$db = Baza::dajDB();
		$godzina .= ":00:00";
		$today = new DateTime($data);
		$today->modify($godzina);
		...
		if (!empty($_POST['date'])) {
								...
								for ($j = 1; $j <= $ilosc_torow; $j++) {
									...
									if ($i == $otwarcie-1) {
										...
										if (czyWolny($today->format("Y-m-d"), $i, $_GET['s2'])) {
											...
											{
												echo "<td>Wolny</td>\n";
												echo "<td><input type='checkbox' name='box_tablica[]' value='".$today->format("Y-m-d").",".$i.",".$_GET['s2'].",".$today->format("Y-m-d")."'></td>\n";
											}
										}
										else
										{
			...
			if (!empty($_POST['box_tablica'])) {
				...
				//Rozdzielam tablicę po przecinkach, 0-Data, 1-Godzina, 2-Tor
					$dane = explode(",", $value);
					echo " ".$dane[1].",";
					rezerwacjaToru($Uzytkownik->getId(), $dane[1], $dane[2], $dane[3]);
				}
				echo "<br />";
